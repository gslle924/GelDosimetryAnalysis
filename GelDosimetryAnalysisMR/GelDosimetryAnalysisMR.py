...
import logging
import GelDosimetryAnalysisMRLogic
import DataProbeLib
import slicer.util
...
class GelDosimetryAnalysisSlicelet(VTKObservationMixin):
  def __init__(self, parent, developerMode=False, widgetClass=None):
    ...
    # Create module logic
    self.logic = GelDosimetryAnalysisMRLogic.GelDosimetryAnalysisMRLogic()
  ...
  def setup_Step1_LoadData(self):
    ...
    self.step1_2_1_1_gradientIterationsSpinBox = qt.QSpinBox() 
    ...
    self.step1_2_1_1_gradientTimeStepSpinBox = qt.QDoubleSpinBox() 
    ...
    self.step1_2_2_1_gradientTimeStepSpinBox.setValue(0.02)
    ...
    self.step1_2_1_1_filterTypeComboBox.connect('currentIndexChanged(int)', self.onFilterTypeChanged) 
  ...
  def createCalibrationCurvesWindow(self):
    ...
    renderWindow = self.calibrationCurveChartView.GetRenderWindow()
    renderWindow.SetSize(800, 550)
    renderWindow.SetMultiSamples(0)
    self.calibrationCurveChartRenderWindow = renderWindow
  ...
  def showCalibrationCurves(self):
    ...
    self.calibrationCurveChartRenderWindow.Render()
    slicer.app.processEvents()
  ...
  def onShowDeltaRVsDoseCurve(self):
    ...
    renderWindow = self.deltaRVsDoseChartView.GetRenderWindow()
    renderWindow.SetSize(800, 550)
    renderWindow.SetMultiSamples(0)
    self.deltaRVsDoseChartRenderWindow = renderWindow
    ...
    slicer.app.processEvents()
    self.deltaRVsDoseChartRenderWindow.Render()
    slicer.app.processEvents()
  ...
  def onExportLineProfiles(self):
    if hasattr(self, "lineProfileData") and self.lineProfileData is not None:
      result = self.logic.exportLineProfileToCSV(self.lineProfileData)
      if result and result.startswith("File saved:"):
        exportedPath = result.split("File saved:\n")[-1]
        qt.QMessageBox.information(None, "Line Profile Export", f"Line profile exported:\n{exportedPath}")
      else:
        qt.QMessageBox.warning(None, "Line Profile Export", result or "Export failed: no data")
    else:
    ...
    try:
      ...
      if cliNode.GetStatus() & cliNode.Completed:
        ...
        if cliNode2.GetStatus() & cliNode2.Completed:
          ...
          bounds = [0.0] * 6
          outputNode.GetRASBounds(bounds)
          center = ((bounds[0] + bounds[1]) / 2.0,
                    (bounds[2] + bounds[3]) / 2.0,
                    (bounds[4] + bounds[5]) / 2.0)

          # Fixed pivot transform
          pivotTransformNode = slicer.mrmlScene.GetFirstNodeByName("PreToPostManualAdjust")
          if pivotTransformNode is None:
              pivotTransformNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLLinearTransformNode", "PreToPostManualAdjust")
          pivotMatrix = vtk.vtkMatrix4x4()
          pivotMatrix.Identity()
          pivotMatrix.SetElement(0, 3, center[0])
          pivotMatrix.SetElement(1, 3, center[1])
          pivotMatrix.SetElement(2, 3, center[2])
          pivotTransformNode.SetMatrixTransformToParent(pivotMatrix)

          # Manual adjustment transform
          self.manualTransformNode = slicer.mrmlScene.GetFirstNodeByName("PreToPostManualAdjustRotation")
          if self.manualTransformNode is None:
              self.manualTransformNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLLinearTransformNode", "PreToPostManualAdjustRotation")
          ...
          self.manualTransformNode.SetAndObserveTransformNodeID(pivotTransformNode.GetID())
          ...
          self.step1_2_1_1_rotationSliders.setMRMLTransformNode(pivotTransformNode)
          self.step1_2_1_1_resampleButton.visible = False
          self.manualTransformTouched = False
          self.mainResampleDone = False
        else:
  ...
  def onManualTransformChanged(self):
      ...
      self.manualTransformTouched = True
      self.mainResampleDone = False
  ...
  def onResampleMeasured(self):
    ...
    referenceNode = self.step1_2_1_preScanSelector.currentNode()
    if referenceNode is None:
        qt.QMessageBox.warning(None, 'Warning', 'No reference volume selected.')
        return
    
    qt.QApplication.setOverrideCursor(qt.QCursor(qt.Qt.BusyCursor))
    
    progressDialog = qt.QProgressDialog("Resampling volume.", "OK", 0, 0)
    progressDialog.setModal(True)
    progressDialog.setMinimumDuration(0)
    progressDialog.show()
    slicer.app.processEvents()
    ...
    try:
        ...
        selectionNode.SetSecondaryVolumeID(referenceNode.GetID())
        appLogic.PropagateVolumeSelection()
        progressDialog.close()
        qt.QApplication.restoreOverrideCursor()
        self.step1_2_1_1_resampleButton.enabled = False
        self.mainResampleDone = True
    
    except Exception as e:
        progressDialog.close()
        qt.QApplication.restoreOverrideCursor()
        qt.QMessageBox.critical(None, 'Error', f'Resampling failed: {str(e)}')
  ...
  def onApplyTransformToR1(self):
    ...
    try:
      ...
      if cliNode.GetStatus() & cliNode.Completed:
        if getattr(self, 'manualTransformTouched', False):
          # Carry over the manual adjustment transform
          manualWorldMatrix = vtk.vtkMatrix4x4()
          self.manualTransformNode.GetMatrixTransformToWorld(manualWorldMatrix)

          r1ManualAdjustNode = slicer.mrmlScene.GetFirstNodeByName("R1ManualAdjustApplied")
          if r1ManualAdjustNode is None:
              r1ManualAdjustNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLLinearTransformNode", "R1ManualAdjustApplied")
          r1ManualAdjustNode.SetMatrixTransformToParent(manualWorldMatrix)

          resampleParameters2 = {
            'inputVolume': outputNode.GetID(),
            'referenceVolume': r1PreNode.GetID(),
            'outputVolume': outputNode.GetID(),
            'pixelType': 'float',
            'warpTransform': r1ManualAdjustNode.GetID(),
            'interpolationMode': 'Linear',
          }
    ...
    try:
      ...
      if cliNode.GetStatus() & cliNode.Completed:
        ...
        if cliNode2.GetStatus() & cliNode2.Completed:
          ...
          # Fixed pivot transform
          pivotTransformNode = slicer.mrmlScene.GetFirstNodeByName("CalibrationPreToPostManualAdjust")
          if pivotTransformNode is None:
              pivotTransformNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLLinearTransformNode", "CalibrationPreToPostManualAdjust")
          pivotMatrix = vtk.vtkMatrix4x4()
          pivotMatrix.Identity()
          pivotMatrix.SetElement(0, 3, center[0])
          pivotMatrix.SetElement(1, 3, center[1])
          ...
          # Manual adjustment transform
          self.calibrationManualTransformNode = slicer.mrmlScene.GetFirstNodeByName("CalibrationPreToPostManualAdjustRotation")
          if self.calibrationManualTransformNode is None:
              self.calibrationManualTransformNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLLinearTransformNode", "CalibrationPreToPostManualAdjustRotation")
          matrix = vtk.vtkMatrix4x4()
          matrix.Identity()
          matrix.SetElement(0, 3, -center[0])
          matrix.SetElement(1, 3, -center[1])
          ...
          self.calibrationManualTransformNode.SetAndObserveTransformNodeID(pivotTransformNode.GetID())
          ...
          self.step1_2_2_1_rotationSliders.setMRMLTransformNode(pivotTransformNode)
          self.step1_2_2_1_resampleButton.visible = False
          self.calibrationManualTransformTouched = False
          self.calibrationMainResampleDone = False
        else:
          qt.QMessageBox.critical(None, 'Error', 'Resampling failed')
      else:
        progressDialog.close()
  ...
  def onCalibrationManualTransformChanged(self):
    ...
    self.calibrationManualTransformTouched = True
    self.calibrationMainResampleDone = False
  ...
  def onResampleCalibration(self):
    ...
    referenceNode = (self.step1_2_2_1_r1PreSelector.currentNode()
                      if self.step1_2_2_1_useGRECheckBox.isChecked()
                      else self.step1_2_2_preScanSelector.currentNode())
    if referenceNode is None:
        qt.QMessageBox.warning(None, 'Warning', 'No reference volume selected.')
        return
    ...
    try:
        ...
        qt.QApplication.restoreOverrideCursor()
        self.step1_2_2_1_resampleButton.enabled = False
        self.calibrationMainResampleDone = True
  ...
  def onCalibrationApplyTransformToR1(self):
    ...
    try:
      ...
      if cliNode.GetStatus() & cliNode.Completed:
        if getattr(self, 'calibrationManualTransformTouched', False):
          manualWorldMatrix = vtk.vtkMatrix4x4()
          self.calibrationManualTransformNode.GetMatrixTransformToWorld(manualWorldMatrix)

          r1ManualAdjustNode = slicer.mrmlScene.GetFirstNodeByName("CalibrationR1ManualAdjustApplied")
          if r1ManualAdjustNode is None:
              r1ManualAdjustNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLLinearTransformNode", "CalibrationR1ManualAdjustApplied")
          r1ManualAdjustNode.SetMatrixTransformToParent(manualWorldMatrix)
...
#
class GelDosimetryAnalysisMR(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    parent.title = "MR Gel Dosimetry Analysis"
...
#
class GelDosimetryAnalysisMRWidget(ScriptedLoadableModuleWidget):
  ...
  def launchSlicelet(self):
    ...
    mainFrame.windowTitle = "MR Gel dosimetry analysis"
    mainFrame.setWindowFlags(qt.Qt.WindowCloseButtonHint | qt.Qt.WindowMaximizeButtonHint | qt.Qt.WindowTitleHint)
    iconPath = os.path.join(os.path.dirname(slicer.modules.geldosimetryanalysismr.path), 'Resources/Icons', self.moduleName+'.png')
...
# ---------------------------------------------------------------------------
class GelDosimetryAnalysisMRTest(ScriptedLoadableModuleTest):
  """
  ...
  def TestSection_02_FinalizeDataLoading(self):
    ...
    try:
      ...
      moduleWidget = slicer.modules.geldosimetryanalysismr.widgetRepresentation().self()
